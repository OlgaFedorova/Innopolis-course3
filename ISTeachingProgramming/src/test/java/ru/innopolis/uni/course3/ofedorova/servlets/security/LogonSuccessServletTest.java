...
import ru.innopolis.uni.course3.ofedorova.servlets.security.LogonSuccessServlet;
...
/**
 * Класс для тестирования LogonSuccessServlet.
 *
...
public class LogonSuccessServletTest {
    @Mock
    ...
    public void whenDoGet() throws Exception {
        when(request.getRequestDispatcher("/security/success-logon.jsp")).thenReturn(dispatcher);

        final LogonSuccessServlet logonSuccessServlet = new LogonSuccessServlet();
        logonSuccessServlet.doGet(request, response);

        verify(request, atLeastOnce()).getRequestDispatcher("/security/success-logon.jsp");
    }
